...
const MONGO_URL = "mongodb+srv://valli:welcome123@cluster0.md1noof.mongodb.net"
...
app.get('/movies/:id', async function(req, res){
    ...
    const movie = await client.db("b33").collection("movies").findOne({id: id})
...
//     const movie = movies.filter((mv) => mv.name === name )  // to get array of objects we can use find & find always give element
    ...
    app.post("/movies", async function (req, res) {
      ...
      console.log(movies);
      const result = await client.db("b33").collection("movies").insertMany(movies);
      res.send(result)
});
  ...
  app.get("/movies", async function (request, response) {
    ...
    const movies = await client.db("b33").collection("movies").find(request.query).toArray(); //to convert cursor to Array
  response.send(movies);

app.delete("/movies/:id", async function (request,response) {
  console.log(request.query);
  const { id } = request.query;
  const movies = await client.db("b33").collection("movies").deleteOne({id: id});
  response.send(movies);
})
  });
